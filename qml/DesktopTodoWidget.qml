...
QtWindow.Window {
    ...
    property var focusSession: null
    ...
    function reload() {
        ...
        try { focusSession = JSON.parse(Backend.focusSessionJson) } catch (error) { focusSession = null }
        todayFocusMinutes = Number(Backend.todayFocusMinutes || 0)
    }
    ...
    function focusActive() {
        if (!focusSession || typeof focusSession !== "object")
            return false
        if (focusSession.cancelled || focusSession.finished)
            return false
        return Number(focusSession.remaining || 0) > 0
    }
    function focusTitle() {
        return String((focusSession && focusSession.title) || "专注任务")
    }
    function focusRemaining() {
        return focusActive() ? Math.max(0, Math.floor(Number(focusSession.remaining || 0))) : 0
    }
    ...
    onCompactModeChanged: if (widget.compactMode) widget.currentSection = 0
