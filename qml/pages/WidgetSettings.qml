...
FluentPage {
    ...
    function load() {
        ...
        autoShow.checked = Boolean(Backend.getSetting("desktopWidgetAutoShow", true))
        compactMode.checked = Boolean(Backend.getSetting("desktopWidgetCompact", false))
    ...
    SettingCard {
        Layout.fillWidth: true
        title: "简洁模式"
        description: compactMode.checked ? "已隐藏分类栏，仅显示待办与专注状态" : "隐藏任务流 / 项目分类，仅保留待办与专注状态"
        icon.name: "ic_fluent_apps_list_detail_20_regular"
        Switch {
            id: compactMode
            onClicked: if (!page.loading) Backend.setSetting("desktopWidgetCompact", checked)
        }
    }
